...
<div class="card-custom">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h5><i class="fas fa-file-alt mr-2"></i>View Request - <?= htmlspecialchars($request->control_no) ?></h5>
        <div>
            <?php if ($request->status == 'approved'): ?>
            <a href="<?= site_url('clockchangerequest/print_request/' . $request->id) ?>" class="btn btn-info btn-sm mr-1" target="_blank">
    ...
    <div class="card-body">
        ...
        <div class="alert alert-<?php
            ...
            elseif ($request->status == 'approved') echo 'success';
            elseif ($request->status == 'rejected') echo 'danger';
            else echo 'secondary';
        ?> mb-4">
            <strong>Status:</strong> <?= ucfirst($request->status) ?>
            <?php if ($request->approved_at): ?>
                | <strong><?= $request->status == 'approved' ? 'Approved' : 'Reviewed' ?> on:</strong> <?= date('M d, Y h:i A', strtotime($request->approved_at)) ?>
            <?php endif; ?>
            <?php if ($request->approval_remarks): ?>
                | <strong>Remarks:</strong> <?= htmlspecialchars($request->approval_remarks) ?>
            <?php endif; ?>
        </div>

        <!-- Form Preview (matching printout) -->
        <div class="fctc-form-container">
            ...
            <div class="fctc-signature">
                <div class="sig-line">
                    ...
                    <?php elseif (isset($approver) && $approver): ?>
                        <?= htmlspecialchars(strtoupper($approver->lastname . ', ' . $approver->firstname)) ?>
                    <?php else: ?>
                        &nbsp;
                    <?php endif; ?>
                </div>
        ...
        <div class="text-center mt-4">
            <a href="<?= site_url('clockchangerequest/cancel/' . $request->id) ?>" class="btn btn-danger" onclick="return confirm('Are you sure you want to cancel this request?')">
